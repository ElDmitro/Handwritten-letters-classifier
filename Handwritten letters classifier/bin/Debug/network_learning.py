...
#Train Step:
train_step = tf.train.AdamOptimizer(4e-4).minimi    ze(cross_entropy)
...
def server_start():
    ...
    im = Image.open("/home/eldmitro/Debug/image3.jpg")
    a = np.array(im)
    a = a.flatten()
    a = a.reshape(1,2352)
    ...
    print get_letter(ans[0])

sess = tf.InteractiveSession()
#Learn(name = "test", train_set = input_data, train_set_labels=input_label, batch_size=10, N_ATTEMPTIONS=20, epochs = 500, sess=sess, testing = True)
#Learn(name = "fullLearn", batch_size=15, N_ATTEMPTIONS=1, epochs=1001, sess=sess, testing=False)
